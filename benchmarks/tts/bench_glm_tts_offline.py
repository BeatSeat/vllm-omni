...
def _build_prompt(
    ...
    return {
        ...
        "model_intermediate_buffer": build_glm_tts_prefill_metadata(
            model, text, ref_text, trust_remote_code=True,
        ),
    }
        ...
        with open(args.jsonl_prompts, encoding="utf-8") as f:
            for line_no, raw_line in enumerate(f, start=1):
                ...
                if not ref_audio or not ref_text:
                    raise ValueError(
                        f"{args.jsonl_prompts}:{line_no} requires both ref_audio and ref_text"
                    )
                specs.append(PromptSpec(
    ...
    if args.warmup_runs:
        ...
        for wi in range(args.warmup_runs):
            ...
            _, ttfp, elapsed, dur = _run_single(
                warmup_spec, request_prefix=f"warmup{wi + 1}", save_outputs=False,
            )
    ...
    for run in range(args.num_runs):
        for pi, spec in enumerate(prompt_specs):
            paths, ttfp, elapsed, dur = _run_single(
                spec, request_prefix=f"sync_run{run + 1}_{pi + 1:03d}",
                save_outputs=True, run_index=run,
            )
...
async def _run_streaming(args) -> list[Path]:
    ...
    async def _collect_one(
        spec: PromptSpec, request_id: str,
    ...
    for run in range(args.num_runs):
        for pi, spec in enumerate(prompt_specs):
            ...
            _emit_offline_metrics(
                request_id=rid, label=spec.label,
                elapsed_s=elapsed, first_audio_elapsed=ttfp,
                audio_duration_s=dur,
            )
...
def parse_args():
    parser = FlexibleArgumentParser(
        description="Offline GLM-TTS benchmark (sync + streaming, voice cloning)"
    )
    ...
    parser.add_argument(
        "--stage-configs-path", type=str, default=str(DEFAULT_STAGE_CONFIG),
