...
def _load_ref_audio() -> tuple[np.ndarray, int]:
    """Load vendored reference audio from tests/assets/."""
    audio, sr = sf.read(str(_ASSET_DIR / "ref.wav"), dtype="float32", always_2d=False)
...
def _load_vocal_audio() -> tuple[np.ndarray, int]:
    """Load vendored vocal audio from tests/assets/."""
...
def test_funcineforge_offline_dubbing_with_face(async_chunk: bool) -> None:
    ...
    face_path = str(_ASSET_DIR / "en_monologue_1.pkl")
...
def test_funcineforge_offline_video_preprocess(async_chunk: bool) -> None:
    ...
    Uses vendored test video from tests/assets/, runs
    ...
    model_dir = _resolve_model_dir()
    video_path = str(_ASSET_DIR / "en_monologue_1.mp4")
