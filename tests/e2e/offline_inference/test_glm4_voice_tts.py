...
def test_offline_text_to_speech_sync() -> None:
    """Sync-mode TTS: text → audio (no async_chunk pipeline)."""
    ...
    omni = Omni(
        model=MODEL,
        stage_configs_path=_get_deploy_config(async_chunk=False),
...
@pytest.mark.omni
@hardware_test(res={"cuda": "L4"}, num_cards=1)
def test_offline_text_to_speech_multilang() -> None:
    """Multi-language TTS: reuses one Omni instance for zh + en."""
    from vllm_omni import Omni
    from vllm_omni.model_executor.models.glm4_voice.glm4_voice import (
        build_glm4_voice_prompt,
    )
    ...
    try:
        for synth_text in texts:
            prompt_text = build_glm4_voice_prompt(synth_text)
            outputs = omni.generate([{"prompt": prompt_text}])

            assert outputs, f"No outputs for: {synth_text[:20]}"
            mm = outputs[0].multimodal_output
            assert mm is not None and "audio" in mm

            audio = _concat_audio(mm["audio"])
            assert audio.size > 0, f"Empty audio for: {synth_text[:20]}"
    finally:
        omni.close()
