...
def test_a_trim_lands_on_the_low_watermark():
    ...
    assert plan.moved_from == align_up(220 - target, BLOCK) == 148
...
def test_unit_aligned_cut_keeps_whole_units():
    # 12 tokens of audio per 1 s unit, first unit right after the prefix.
    units = [12] * 8
    geometry = _geometry(window_tokens=30, high_watermark_tokens=40)
    boundaries = unit_starts(geometry, units)
    assert boundaries[0] == geometry.prefix_tokens
    assert boundaries[-1] == 10 + 7 * 12

    plan = plan_position_reanchor(geometry, computed_tokens=10 + sum(units), pending_tokens=12, unit_tokens=units)
    assert plan is not None
    aligned = {align_up(b, BLOCK) for b in boundaries}
    # Landed on a block-aligned unit start, so no unit was split in half.
    assert plan.moved_from in aligned
...
def test_reanchor_declines_when_there_is_nothing_left_to_reclaim():
    ...
    # caller has to finish the session with context_length_exceeded instead.
    assert plan_position_reanchor(geometry, computed_tokens=100, pending_tokens=20, target_tokens=160) is None
    # A target that leaves no room between the sink's last page and the cut: the
    # smallest legal cut already keeps the whole sequence, so no shift exists.
    assert plan_position_reanchor(geometry, computed_tokens=100, pending_tokens=20, target_tokens=108) is None
...
def test_a_long_session_trims_once_per_spare_window_not_once_per_append():
    ...
    assert all(gap <= spare // append + 1 for gap in gaps[1:])
