...
from vllm_omni.model_executor.models.common.tts_sampling import ras_sample_one
...
def test_glm_tts_tokenizer_loader_falls_back_to_repo_subfolder(monkeypatch: pytest.MonkeyPatch) -> None:
    marker = object()
    calls = []
    ...
    def fake_from_pretrained(path, **kwargs):
        ...
        if (
            path == "zai-org/GLM-TTS"
            and kwargs.get("subfolder") == "vq32k-phoneme-tokenizer"
            and kwargs.get("use_fast") is False
        ):
            return marker
        raise RuntimeError("tokenizer unavailable")

    monkeypatch.setattr(glm_tts_module.AutoTokenizer, "from_pretrained", fake_from_pretrained)

    tok = glm_tts_module.load_glm_tts_tokenizer(
        "/tmp/hub/models--zai-org--GLM-TTS/snapshots/abc/vq32k-phoneme-tokenizer",
        model_name_or_path="/tmp/hub/models--zai-org--GLM-TTS/snapshots/abc/llm",
        trust_remote_code=True,
    )

    assert tok is marker
...
def test_glm_tts_prompt_updates_append_single_audio_suffix() -> None:
    ...
    out_mm_kwargs = {
        "audio": [
            {
                "prompt_speech_token_len": SimpleNamespace(
                    data=[torch.tensor([5], dtype=torch.long)]
                )
            }
        ]
    }
...
def test_glm_tts_preprocess_initializes_generation_bounds_before_sampling() -> None:
    ...
    assert model._glm_tts_min_audio_tokens == 54
    assert model._glm_tts_max_audio_tokens == 540
    assert model._glm_tts_decode_step == 0


def test_glm_tts_compute_logits_only_masks_eoa_before_min_len() -> None:
    model = object.__new__(GLMTTSForConditionalGeneration)
    model.model_stage = "glm_tts"
    model.lm_head = object()
    model.logits_processor = lambda _head, hidden: hidden.clone()
    model._eoa = 2
    model._glm_tts_min_audio_tokens = 5
    model._glm_tts_max_audio_tokens = 20
    model._glm_tts_decode_step = 0

    logits = model.compute_logits(torch.tensor([[1.0, 2.0, 3.0, 4.0]]))
...
def test_glm_tts_preprocess_recovers_prefill_lengths_from_mm_features(
    ...
    assert model._glm_tts_min_audio_tokens == 22
    assert model._glm_tts_max_audio_tokens == 220
    assert model._glm_tts_decode_step == 0
