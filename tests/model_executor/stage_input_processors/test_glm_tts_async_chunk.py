...
def test_ar_to_dit_async_chunk_emits_cumulative_prefixes() -> None:
    ...
    assert payloads[0].meta.token_offset == 0
    assert payloads[0].meta.block_pattern == [25]
    assert _audio_codes(payloads[1]) == list(range(50))
    assert payloads[1].meta.token_offset == 25
    assert payloads[1].meta.block_pattern == [25]
    assert _audio_codes(payloads[2]) == list(range(75))
    assert payloads[2].meta.token_offset == 50
    assert payloads[2].meta.block_pattern == [25]
...
def test_ar_to_dit_async_chunk_keeps_progressive_block_pattern_fixed() -> None:
    ...
    assert payloads[0].meta.chunk_sizes_history == [25]
    assert payloads[0].meta.block_pattern == [25, 50, 200]
    assert final_payload is not None
    assert final_payload.meta.chunk_sizes_history == [25, 5]
    assert final_payload.meta.block_pattern == [25, 50, 200]
...
def test_ar_to_dit_async_chunk_terminal_flushes_boundary_prefix() -> None:
    ...
    assert final_payload.meta.token_offset == 50
...
def test_ar_to_dit_async_chunk_finished_flushes_partial_unsent_tail() -> None:
    ...
    assert final_payload.meta.token_offset == 25
