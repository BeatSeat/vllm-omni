...
def get_vocos_mel_spectrogram(
    ...
    *,
    ...
    assert waveform.ndim == 2, f"vocos mel expects 2D waveform [B,N], got shape {waveform.shape}"
...
def get_bigvgan_mel_spectrogram(
    ...
    *,
    ...
    assert waveform.ndim == 2 and waveform.shape[0] == 1, (
        f"bigvgan mel expects [1,N] waveform, got shape {waveform.shape}"
    )
    return _bigvgan_mel(
        waveform,
        n_fft=n_fft,
...
class MelSpec:
    ...
    def __call__(self, wav: torch.Tensor) -> torch.Tensor:
        ...
        assert wav.ndim == 2 and wav.shape[0] == 1, f"MelSpec expects [1,N] wav, got shape {wav.shape}"
        ...
        assert mel.shape[-1] == self.config.n_mel_channels, (
            f"mel channels mismatch: got {mel.shape[-1]}, expected {self.config.n_mel_channels}"
        )
        return mel
...
def _load_audio_to_np(x: str | bytes | tuple[Any, int]) -> tuple[np.ndarray, int]:
    import soundfile as sf

    if isinstance(x, tuple) and len(x) == 2:
        # Already-decoded (waveform, sample_rate) from the serving layer,
        # which resolves the locator through its MediaConnector policy.
        audio = np.asarray(x[0], dtype=np.float32)
        sr = int(x[1])
        ...
        # Local file path, optionally as a file:// URI (allowed by the
        # serving validator). soundfile (a declared dependency) covers
        # wav/flac/ogg; mp3/aac would need an extra decoder, so fail
        # loudly instead of silently pulling in librosa (banned by ruff
        # TID251).
        if x.startswith("file://"):
            x = urlparse(x).path
        if not Path(x).is_file():
            raise FileNotFoundError(f"ref_audio not a URL, base64, or existing file: {x!r}")
...
def process_audio(
    audio_path: str | bytes | tuple[Any, int],
    *,
    mel_spec: MelSpec,
    ...
    Args:
        audio_path: Reference audio path / URI / bytes, or an
            already-decoded ``(waveform, sample_rate)`` tuple as provided by
            the serving layer after MediaConnector resolution.
    ...
    assert mel.dtype in (torch.float32, torch.float64), f"mel dtype must be float32/64, got {mel.dtype}"
    mel = mel.float()
...
def _load_vocos_vocoder(*, sample_rate: int, device: str) -> Any:
    ...
    try:
        ...
        raise ImportError("vocos is required for vocoder_name='vocos'. Install with: pip install vocos")

    repo_id = "charactr/vocos-mel-24khz"
    config_path = hf_api().hf_hub_download(repo_id=repo_id, filename="config.yaml")
    model_path = hf_api().hf_hub_download(repo_id=repo_id, filename="pytorch_model.bin")
