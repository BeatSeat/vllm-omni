...
class TextEmbedding(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.text_embed = nn.Embedding(text_num_embeds + 1, output_dim)  # +1 for padding
    ...
    def forward(self, text: torch.Tensor, aim_seq_len: int) -> torch.Tensor:
        batch, text_len = text.shape[0], text.shape[1]
        if self.length_align == "fill":
            text = text[:, :aim_seq_len]
            text = F.pad(text, (0, aim_seq_len - min(text_len, aim_seq_len)), value=0)
...
class GLMTTSDiT(nn.Module):
    ...
    def forward(
        ...
        text_embed = self.text_emb_layer(text, seq_len).to(model_dtype)
