...
class GLMTTSDiT(nn.Module):
    ...
    def _create_block_causal_mask(
        ...
        for i in range(seq_len):
            # Find which block position i belongs to
            for j, acc_val in enumerate(accum):
                if i < acc_val:
                    cur_block_size = extended[j]
                    last_end = accum[j - 1] if j > 0 else 0
                    break
            # Look-ahead: positions near the end of a block can see into next
            idx_in_block = i - last_end
            look_future = cur_block_size - 1 - idx_in_block
            min_future = cur_block_size // 2 if cur_block_size <= 172 else int(cur_block_size / 1.5)
            delta = max(0, min_future - look_future)
            aim_len = last_end + cur_block_size + delta
            mask[i, : min(aim_len, seq_len)] = True
