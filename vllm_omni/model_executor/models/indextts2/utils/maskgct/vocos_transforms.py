...
class IMDCT(nn.Module):
    ...
    def forward(self, mdct_coefs: torch.Tensor) -> torch.Tensor:
        """
        Apply the Inverse Modified Discrete Cosine Transform (IMDCT)
        to the input MDCT coefficients.

        Args:
            mdct_coefs (Tensor): Input MDCT coefficients of shape
                (B, L, N), where B is the batch size, L is the number
                of frames, and N is the number of frequency bins.

        Returns:
            Tensor: Reconstructed audio waveform of shape (B, T),
                where T is the length of the audio.
        """
        B, L, N = mdct_coefs.shape
        Y = torch.zeros((B, L, N * 2), dtype=mdct_coefs.dtype, device=mdct_coefs.device)
        Y[..., :N] = mdct_coefs
        Y[..., N:] = -1 * torch.conj(torch.flip(mdct_coefs, dims=(-1,)))
...
class IMDCTSymExpHead(FourierHead):
    ...
    Args:
        ...
        clip_audio (bool, optional): Whether to clip the audio output
            within the range of [-1.0, 1.0]. Defaults to False.
    """
